...
export function OAuthButtons({
  ...
  const handleOAuthSignIn = async (providerId: string) => {
    ...
    try {
      ...
      setError(t("auth:errors.oauth_generic"));
    } finally {
  ...
  const getErrorMessage = (error: string): string => {
    const errorMap: Record<string, string> = {
      ...
      AccessDenied: t("auth:errors.access_denied"),
    };

    return errorMap[error] || t("auth:errors.oauth_generic");
  };

  const getButtonText = (provider: OAuthProvider): string => {
    const actionKey = action === "signin" ? "signin" : "signup";
    return t(`auth:oauth.${actionKey}_with`, { provider: provider.name });
  };
    ...
    <div className={cn("space-y-3", className)}>
      ...
      {enabledProviders.length > 0 && (
        <p className="text-xs text-center text-muted-foreground">
          {action === "signup"
            ? t("auth:oauth.signup_agreement")
            : t("auth:oauth.signin_agreement")}
        </p>
      )}
    </div>
  );
